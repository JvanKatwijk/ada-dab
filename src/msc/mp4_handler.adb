...
--	Now for real: first check the firecode
	...
	if not firecode_Checker. check (fcVector)
	then
...
--	the superframe
	processSuperFrame (Object,
	                   ...
	                   result);
	if not result
	then	-- we will try it again, next time
	   Object. blocksInBuffer	:= 4;
	   Object. frameErrors		:= Object. frameErrors + 1;
	   return;
	end if;
...
--	Now first the crc check
	      if dabPlus_crc (outVector, au_start (i), aac_frame_length)
	      then
	         theAU (0 .. Integer (aac_frame_length - 1)) :=
	                outVector (Integer (au_start (i)) ..
	                          Integer (au_start (i) + aac_frame_length - 1));
...
begin
	   ...
	   data		:= Shift_Left (uint16_t (msg (Integer (start + i))), 8);
           for j in Reverse 1 .. 8 loop
              if ((data xor accumulator) and 16#8000#) /= 0
	      then
